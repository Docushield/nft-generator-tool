...
import { BsArrowDown, BsArrowUp, BsFillCheckCircleFill, BsFillXCircleFill } from 'react-icons/bs';
import { useAppContext } from '@/state/context';
import { updateOrganizeData, updateCurrentLayers } from '@/state/actions';

export default function LayerItem({ data, moveItem, deleteItem, editItem, className, ...props }) {
  const { state, dispatch } = useAppContext();
  const { organizeData, currentAttributeId, currentLayers } = state;
  const cs = classNames(['flex items-center p-2 bg-white w-full', className]);
  ...
  return (
    <div className={cs} {...props}>
      ...
      {editing ? (
        ...
        <div
          className="uppercase border h-14 text-center rounded-md m-2 py-2 px-5 md:py-2 md:px-5 w-full text-gray shadow border-zinc-500 bg-[#FFEDBD] flex justify-center items-center"
          onClick={(e) => {
            setEditing(true);
          }}
        >
